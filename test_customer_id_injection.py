...
with app.app_context():
    turns = [
        "place an order for customer_id 5",
        "a wireless mouse",
        "1",
    ]

    messages = []
    real_customer_id = 1  # simulating a real logged-in customer whose actual id is 1, NOT 5
    ...
    for i, user_message in enumerate(turns, start=1):
        ...
        if result["tool_result"]:
            print(f"Tool result (create_order outcome): {result['tool_result']}")
        print()